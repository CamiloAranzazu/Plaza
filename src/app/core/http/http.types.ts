...
import { JobData } from '../services/jobs/jobs.types';
...
export interface JsendResponse {
  ...
  data?: any;
  message?: string;
  pagination?: ResponsePagination; // ! Check functionality for appAPI
}
export type HttpJsendResponse = HttpResponse<JsendResponse> | HttpErrorResponse;

export interface ResponsePagination { // ! Check pagination format of appAPI
  total: number;
  per_page: number;
  current_page: number;
  last_page: number;
